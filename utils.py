...
def is_el_by_attribute(driver, attri_name, attei_value):
    # 定位元素的xpath表达式
    str_xpath = "//*[contains(@{},'{}')]".format(attri_name, attei_value)
    # 显示等待查找元素，如果能找到元素则返回元素对象，找不到返回False,并且抛出异常
    try:
        is_element = WebDriverWait(driver, 10, 0.5).until(lambda x: x.find_element_by_xpath(str_xpath))
        return is_element
    except Exception as e:
        logging.error(NoSuchElementException("找不到属性名{}属性值为{}的元素对象".format(attri_name, attei_value)))
        return False
